 ...
 */
require_once dirname(__FILE__) . '/RSA/ErrorHandler.php';

/**
 * loader for math wrappers
 */
require_once dirname(__FILE__) . '/RSA/MathLoader.php';

/**
 * helper class for mange single key
 */
require_once dirname(__FILE__) . '/RSA/Key.php';

/**
 * helper class for manage key pair
 */
require_once dirname(__FILE__) . '/RSA/KeyPair.php';

require_once dirname(__FILE__) . '/RSA/PRNG.php';
...
class Crypt_RSA extends Crypt_RSA_ErrorHandler
{
     ...
     * @access public
     */
    public function __construct($params = null, $wrapper_name = 'default', $error_handler = '')
    //function Crypt_RSA($params = null, $wrapper_name = 'default', $error_handler = '')
    {
        // set error handler
        $this->setErrorHandler($error_handler);
        // try to load math wrapper
        $mathLoader = new Crypt_RSA_MathLoader();
        $obj = $mathLoader->loadWrapper($wrapper_name);
    ...
    /**
     * Encrypts data and returns hex string
     *
     * @param string $plain_data
     * @param Crypt_RSA_Key $key
     * @return string
     */
    function encryptHex($plain_data, $key = null)
    {
    	return $this->_math_obj->bin2hex($this->encryptBinary($plain_data, $key));
    }
    ...
    function encryptBinary($plain_data, $key = null)
    {
        ...
        $plain_data = $this->pkcspad($plain_data, ($this->_math_obj->bitLen($this->_math_obj->bin2int($key->getModulus())) + 7) >> 3);
    ...
    /**
     * Pads string with random numbers
     *
     * @param string $s
     * @param int $n
     * @return string
    ...
    function pkcspad($s, $n)
    {
		...
		$bytes = array();	
		$i = strlen($s) - 1;
		while($i >= 0 && $n > 0) $bytes[--$n] = ord($s[$i--]);
		$bytes[--$n] = 0;
		$x = array();
		while($n > 2) { // random non-zero pad
			$x[0] = 0;
			while($x[0] == 0) $this->nextBytes($x);
			$bytes[--$n] = $x[0];
		}
		$bytes[--$n] = 2;
		$bytes[--$n] = 0;
	...
	/**
	 * Gets next random bytes
	 *
	 * @param array $arr
	 */
	private function nextBytes(array & $arr)
	{
		for ($i = 0; $i < count($arr); ++$i)
		{
			$arr[$i] = $this->rng_get_byte(); 
		}
	}
	...
	{
		if(!isset($this->rng_state)) 
		{
			$this->rng_seed_time();
			$this->rng_state = new PRNG();
			$this->rng_state->init($this->rng_pool);
			for($this->rng_pptr = 0; $this->rng_pptr < count($this->rng_pool); ++$this->rng_pptr)
	...
	/**
	 * seeds random number generator by current time
	 *
	 */
	private function rng_seed_time() 
	{
		$this->rng_seed_int(time());
	}
	
	/**
	 * Seeds random number generator with int value
	 *
	 * @param int $x
	 */
	private function rng_seed_int($x) 
	{
		...
		if($this->rng_pptr >= PRNG::rng_psize) $this->rng_pptr -= PRNG::rng_psize;
	}
    
    /**
     * Unpads string after decryption
     *
     * @param string $value
     * @param int $n
     * @return string
     */
    function pkcsunpad($value, $n)
    {
		$i = 0;
		$byte = $this->_math_obj->toByteArray($value);
		while($i < count($byte) && $byte[$i] == 0) ++$i;
		if(count($byte) - $i != $n - 1 || $byte[$i] != 2)
			return null;
		++$i;
		while($byte[$i] != 0)
			if (++$i >= count($byte)) return null;
		$ret = "";
		while(++$i < count($byte))
			$ret .= chr($byte[$i]);
		return $ret;
    ...
    /**
     * Decrypts data represented with hex value
     *
     * @param string $enc_data
     * @param Crypt_RSA_Key $key
     * @return string
     */
    function decryptHex($enc_data, $key = null)
    {
    	return $this->decryptBinary($this->_math_obj->hex2bin($enc_data), $key);
    }
...
function decryptBinary($enc_data, $key = null, $key_len = null, $wrapper_name = null, $error_handler = null)
...
if (is_null($key)) {
    ...
    $keyInstance = new Crypt_RSA_Key($key_len, $wrapper_name, $error_handler);
    $keyIsValid = $keyInstance->isValid($key);
}
if (!$keyIsValid) {
...
*/
        ...
        return /*substr($result, 0, -1)*/ $this->pkcsunpad($result, ($this->_math_obj->bitLen($this->_math_obj->bin2int($key->getModulus())) + 7) >> 3);
    }
